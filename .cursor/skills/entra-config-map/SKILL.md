...
5. `src/entraAuth.js` and `apps/api/src/lib/jwtVerify.ts` (how frontend requests tokens and how API validates them).
...
2. Build cross-checks:
   ...
   - API issuer must match token issuer format accepted by backend (`v2.0` tenant/ciam issuer).
   - API base URL in SPA must target deployed Function App host.
...
## Location Labels
Use these exact location labels in the table:
...
- Keep terminology consistent: use `SPA app`, `API app`, `scope`, `audience`, `issuer`.
