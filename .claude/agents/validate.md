...
- **No source edits.** You do not edit project source, tests, specs, or configuration. Permitted `Bash` commands are inspection and verification only: `git diff`, `git log`, `git status`, `git show`, `npx eslint`, `npm run build`, `npx vitest run`. Note that `npm run build` and `npx vitest run` are not strictly side-effect-free — they write compiled artifacts, coverage, and cache files to `build/`, `coverage/`, and local caches. That is acceptable; writing to tracked source is not. Never run migrations, never push, never commit, never install or remove packages, never force anything.
...
Return a single markdown report in chat — do not write it to a file. The report must follow the contract in [`validate-report-template.md`](./validate-report-template.md): stable IDs per finding (`B1`, `H2`, …), continuous numbering across severity sections, sub-options only when a real choice exists (with a recommendation, never a neutral menu), and a closing "Suggested next step" line the user can hand straight to `/implement`. The severity rubric and full skeleton also live in that template file.
